...
{synopt :{opt u}}estimates of (technical or cost) inefficiency via {it:E}(u|e) (Jondrow et al., 1982){p_end}
{synopt :{opt su}}estimates of spatial-correected inefficiency{p_end}
{synopt :{opt ste}}estimates of spatial-correected efficiency{p_end}
{synoptline}
{p2colreset}{...}
{p 4 6 2}
...
while when the {cmd:spsfe} command is used to estimate cost frontiers, {cmd:predict} will provide the post-estimation of cost (in)efficiency. 
It is worth noting that {cmd:spsfe} and the related {cmd:predict} command follow the definitions of technical and cost (in)efficiency given in Kumbhakar and Lovell (2000).{p_end}
